...
class TractAnomalySingleSubID(gDataset):
    def __init__(self,
                 sub_file,
                 root_dir,
                 labels_name,
        ...
        if with_gt:
            self.labels = []
            for sub in subjects:
                pattern_lab_fn=os.path.join(self.root_dir, sub, sub + '*' + labels_name + '*'+ bundle_name + '*' )
    ...
    def getitem(self, idx):
        ...
        pattern_data_fn=os.path.join(self.root_dir, sub, sub + '*' +  self.data_name + '*'+ self.bundle_name + '*' + self.data_ext)
